 ...
 * @param {Object[][]} schedule - a 2D array of courses, where each inner array represents a semester. Each course has the form: course: {code: courseCode, prio: priority, index: index in courseCodeList}
 * @param {string[]} changedCourses - an array of course codes, representing the courses to be checked
...
const courseMissingDependencyCheck = (schedule, changedCourses, dependencyGraph, courseArray) => {
    ...
    //UOS checking
    changedCourseIndexes.forEach(changedCourseIndex => {
        if (changedCourseIndex[1].us) conflicts[changedCourseIndex[0]].push("uos")
    })
